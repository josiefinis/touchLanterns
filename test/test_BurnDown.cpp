...
//
//
void printSummaryData( uint8_t idx )
{
    std::cout << "\t"; 
...
void printLifeTestData( uint8_t idx, long t )
// Print brightness and reference every 82 s ( in test timeframe, i.e. every 82 ms in real time ).
{
    std::cout << "\t"; 
    int pauseUntil = millis() + 10;
    while ( millis() < pauseUntil ) { }
    ...
    {
        Lantern& lantern = collection[ idx ];
        //std::cout << bridge.getLightRegister( lantern ) << "/" << ( int ) bridge.getReference( lantern );
        printBrightness( collection.getBrightness( idx ), bridge.getReference( lantern ) );
    }
}
...
void runTestSequence( Instruction sequence[], uint8_t verbose=0 ) 
{
    ...
    {  
        ...
        if ( verbose and t % 0x100 == 0 ) { 
            int secs = ( currentMillis - t0 ) / 1000;
            int mins = secs / 60;
            int hrs = secs / 3600;
            std::cout << std::endl << std::dec << t; 
            std::cout << "\t" << hrs << ":" << mins%60 << ":" << secs%60 << "\t" << std::hex;
        }
        ...
        for ( uint8_t idx=0; idx<16; idx++ )
        {
            ...
            if ( verbose == 1 ) { printSummaryData( idx ); }
            if ( verbose == 2 ) { printLifeTestData( idx, t ); }
        }
        while ( time == t )
        {
            Lantern& lantern = collection[ idx ];
...
void test_burnDown( uint8_t verbose=0 ) {
    ...
    Instruction sequence[] = {
        ...
        , Instruction( 90000   , END_TEST )
        , Instruction( 91000   , END_TEST )
    };
    runTestSequence( sequence, verbose );
}
